...
走行線は**モードで色分け**（EV/直列/直結）・**車速で線幅**・白フチ付きで、下地の道路色に埋もれない。**📊 サマリー** ボタンで走行距離・時間（総／走行）・平均車速（総／走行中）・電費（∫Peng+∫Pbat／距離）、消費エネルギーの内訳（消費＝駆動+AUX、うち回生を除いた分が電費）、走行中の EV/直列/直結 配分を時間・距離の2軸でスロープ図表示する（各モードの時間比と距離比をリボンで結び、シフトが分かる）。

再生バー（再生/停止・シーク・速度 1〜10×）で任意位置へ。GPX と .txt は時刻（UTC）で自動結合する。GPS ログはモニター画面の **[GPS] トグル**を ON にして録画すると出力される。
